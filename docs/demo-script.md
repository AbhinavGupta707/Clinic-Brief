...
Target length: 2 minutes 30 seconds or less.
...
Point out the synthetic documents, deterministic facts, and missing-context questions.

## 0:45-1:15 - Review to brief

Open `/cases/sample-preop/review`, then `/cases/sample-preop/brief`.

Show confidence/source provenance on review, then the brief modes:
...
Say: "The same reviewed facts become the right version for the appointment."
...
Say: "Rehearsal asks one preparation question at a time and refuses medical advice."
...
Say: "Novus events are limited to mode, counts, confidence bands, and brief type. The sanitizer drops raw health text, medication names, symptom names, file names, source quotes, and identifiers."
